...
$header = '<div class="prospect-content__table--row title fontDemi">
  ...
  foreach ($fieldsData as $key) {
    ...
    if ($map != 'EMAIL' && $map != 'COMPANY' && $map != 'LAST_NAME' && $map != 'FIRST_NAME' && $map != 'WEBSITE' && $isShow) {
      $header .= '<div class="row-table"><span class="row-table__text">' . __(str_replace("_", " ", ucfirst(strtolower($map))), $this->plugin_name) . '</span></div>';
    }
  }
...
if ($getstatus->status != 'ERROR' && $data->api_key != '') :
  foreach ((array)$getjsonprospects as $prosp) :
    ...
    $emailCol .= '<div class="prospect-content__table--item">
      <div class="row-table"><span class="row-table__text">' . (empty($email) ? '—' : $email) . '</span></div>
    </div>';
    ...
    $additionalCol .= '<div class="prospect-content__table--row">
      ...
      <div class="row-table fontDemi"><span class="row-table__text">' . (empty($firstName) && empty($lastName) ? '—' : $firstName . " " . $lastName)  . '</span></div>
      <div class="row-table"><span class="row-table__text">' . (empty($company) ? '—' : $company) . '</span></div>
      <div class="row-table"><span class="row-table__text">' . (empty($website) ? '—' : $website) . '</span></div>';
      ...
      foreach ($fieldsData as $key) {
        ...
        if ($map != 'EMAIL' && $map != 'COMPANY' && $map != 'LAST_NAME' && $map != 'FIRST_NAME' && $map != 'WEBSITE' && $isShow) {
          ...
          $additionalCol .= '<div class="row-table"><span class="row-table__text">' . (empty($field) ? '—' : $field) . '</span></div>';
        }
      }
